...
@pytest.mark.parametrize(
    "in_channels, strides, coord_level",
    [
        ([12, 24, 48], [1, 2, 4], None),
        ([12, 24, 48], [1, 2, 4], 0),
        ([12, 24, 48], [1, 2, 4], 1),
...
def test_solo_head(
    in_channels: list[int], strides: list[int], coord_level: Optional[int]
) -> None:
    hidden_channels = 64
    base_resolution = 512
    out_channels = 3 * 3
    ...
    features = [
        ...
        for (c, s) in zip(in_channels, strides)
    ]
    head = Head(
...
def test_csp_up_head(
    in_channels: list[int], strides: list[int], use_cord: bool
) -> None:
    out_channels = 16
    hidden_channels = 64
    size = 128
    inputs = [
        torch.rand(2, c, size // r, size // r) for c, r in zip(in_channels, strides)
    ]
    head = CSPUpHead(
        in_channels=in_channels,
        out_channels=out_channels,
